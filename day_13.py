...
buses = str(data[1]).split(",")
# List with numbers of bus lines
buses = [int(x) for x in buses if x != "x" and x != ","]

# Dictionary with bus lines and earliest time
available_times = {}

for bus in buses:
	# range starting from 0, going to time + highest bus and skipping according 
	# to bus value
	for i in range(0, time + bus +1, int(bus)):

		# Find the first value above or equal to time and then break
		if i >= time:
			available_times[bus] = int(i)
			break


# Need a separate list with just the times
all_times = []
for available in available_times:
	all_times.append(available_times[available])

# Smallest value is also the earliest time available
earliest = min(all_times)

wait = earliest - time


# Find name of bus line with smallest value
bus_line = 0
for bus in available_times:
	if available_times[bus] == earliest:
		bus_line += int(bus)

# Multiply waiting time with bus line
result = wait * bus_line
...
with open(file) as f:
	data = f.readlines()
	data = [dat.rstrip() for dat in data]
...
for element in example1:
	if element != "x":
		dictionary[f"bus_{n}"] = (element, n)
	n += 1

print(dictionary)

# Create ranges with adequate steps which all starts from 0 + offset

def infinite_list_bus(starting_point, key_name):
	for i in range(starting_point,1_000_000,int(dictionary[key_name][0])):
		yield i

print(dictionary["bus_0"][1])

# generator_list = []
# i = 0
# for bus in dictionary:
# 	bus_generator = infinite_list_bus(bus)
# 	generator_list.append((bus_generator, dictionary[bus][1]))
# 	i += 1

first_generator = infinite_list_bus(3, "bus_0")

for num in first_generator:
	print(num)



if 1068781 in first_generator:
	print("Ciao")
else:
	print("Bau")
...
# if second bus = first bus + offset and third bus ecc ecc
