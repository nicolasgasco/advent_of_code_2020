...
def find_first_not_sum(list, limit):
	"""Find the first value which is not sum of the n before"""

	all_results = []
	i = limit

	# Never consider the preamble range too
	while i < len(lines) and i > (limit - 1):

		
		# Range for preamble
		preamble = lines[i-limit:i]

		# Set to avoid repetitions
		all_sums = set()

		# Only check in preamble
		for z in range(len(preamble)):

			# I need the original numbers
			for num in preamble:
				if preamble[z] != num:
					sum_preamble = int(preamble[z]) + int(num)
					all_sums.add(sum_preamble)					

		if int(lines[i]) not in all_sums:
			all_results.append(lines[i])
...
result = int(find_first_not_sum(lines,25))
...
for num in lines:
	...
	# We don't cannot sum the number with itself (false positive too)
	if num != result:

		# Tracking sum and single numbers
		nums_list = []

		# Changed depending where it starts
		i = lines.index(str(num))

		# Go on till you find something or there are no numbers
		while i < len(lines) and sum(nums_list) != result:

			nums_list.append(int(lines[i]))
			i += 1
